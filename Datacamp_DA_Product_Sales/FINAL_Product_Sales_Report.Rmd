...
paste0("The total of customers is ", sum(table(data_val$sales_method)))
paste0("The total of groups are ", nlevels(data_val$sales_method))

# Confirmation of 15,000 different values and no duplicated data in customer_id 
paste0("The total of individual customers is ", nlevels(as.factor(data_val$customer_id)))
paste0("The total of duplicates is ", sum(duplicated(data_val$customer_id)))

# Confirmation of 50 different states in the registration in state
data_val$state <- as.factor(data_val$state)
paste0("The total of states is ",nlevels(data_val$state))
```
...
data_val %>%
  ...
  summarize(
    ...
    'SD of Revenue' = sd(revenue)) %>% 
  rename('Sales Method' = sales_method) %>% 
  mutate(across(where(is.numeric), ~round(.x, 2))) %>%  # Redondear para legibilidad
  kable(format = "latex", booktabs = TRUE, caption = "Summary table of revenue") %>%
  kable_styling(latex_options = c("striped", "hold_position", scale_down = TRUE))
...
data_val %>%
  ...
  summarize(
    'N. of customers' = n(), 
    'Mean of  products sold' = mean(nb_sold), 
    'Median of products sold' = median(nb_sold),
    'SD of number of products sold' = sd(nb_sold)) %>%
  rename('Sales Method' = sales_method) %>% 
  mutate(across(where(is.numeric), ~round(.x, 2))) %>%  # Redondear para legibilidad
  kable(format = "latex", booktabs = TRUE, caption = "Summary table of number of new products sold") %>%
  kable_styling(latex_options = c("striped", "hold_position", scale_down = TRUE, float_position = "H"))

data_val %>%
  group_by(sales_method) %>%
  summarize(
     'Number of customers' = n(), 
     'Mean of years' = mean(years_as_customer), 
		 'Median of years' = median(years_as_customer), 
		 'SD of years' = sd(years_as_customer)) %>%
  rename('Sales Method' = sales_method) %>% 
  mutate(across(where(is.numeric), ~round(.x, 2))) %>%  # Redondear para legibilidad
  kable(format = "latex", booktabs = TRUE, caption = "Summary table of years as a customer") %>%
  kable_styling(latex_options = c("striped", "hold_position", scale_down = TRUE, float_position = "H"))
...
data_val %>%
  ...
  summarize(
     'Number of customers' = n(), 
     'Mean of visits' = mean(nb_site_visits), 
		 'Median of visits' = median(nb_site_visits), 
		 'SD of visits' = sd(nb_site_visits)) %>% 
  mutate(across(where(is.numeric), ~round(.x, 2))) %>%  # Redondear para legibilidad
  kable(format = "latex", booktabs = TRUE, caption = "Summary table of number of website visits") %>%
  kable_styling(latex_options = c("striped", "hold_position", scale_down = TRUE, float_position = "H"))
...
```{r}
# Tabla 1: Revenue absoluto

revenue_summ_fmt <- revenue_summ %>%
  mutate(across(where(is.numeric), ~ scales::comma(.x, accuracy = 1))) %>%
  mutate(week = replace(week, n(), "Total"))

kable(revenue_summ_fmt, format = "latex", booktabs = TRUE, caption = "Revenue over weeks") %>%
  kable_styling(latex_options = c("striped", "hold_position"))

revenue_summ_prop_table <- revenue_summ_prop %>% 
  mutate(week = replace(week, n(), "Total"))

# Tabla 2: Proporciones de revenue
kable(revenue_summ_prop_table, format = "latex", booktabs = TRUE, caption = "Proportion of revenue over weeks") %>%
  kable_styling(latex_options = c("striped", "hold_position"))
...
amr <- bind_rows(adj_rev, adj_rev_tot) %>% 
  mutate(week = replace(week, n(), "Total"))

amr

kable(amr, format = "latex", booktabs = TRUE, caption = "Adjusted Mean Revenue Over Time") %>%
  kable_styling(latex_options = c("striped", "hold_position", scale_down = TRUE, float_position = "H"))
```
...
adj_rev_mean <- data_val %>%
 ...
 summarize('Mean' = mean(adjusted_mean), 
		  'Standard Deviation' = sd(adjusted_mean))
adj_rev

kable(adj_rev_mean, format = "latex", booktabs = TRUE, caption = "Adjusted Mean Revenue for Email + Call") %>%
  kable_styling(latex_options = c("striped", "hold_position", scale_down = TRUE, float_position = "H"))
```
